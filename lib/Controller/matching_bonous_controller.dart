...
class MatchingBonousController extends ChangeNotifier {
  String _balance = "0";
  List<MatchingBonusM> _list = [];
  String get matchingBonousbalance => _balance;
  List<MatchingBonusM> get matchingBonouslist => [..._list];

  setmatchingBonusMbalance(String balance) {
    _balance = "0";
    _balance = balance;
    notifyListeners();
  }

  Future<void> checkmatchingBonusbalance() async {
    String token = sharedPreferences.getString('token') ?? "";
    final url = Uri.parse(Api.matchingBonousBalance);
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if (status) {
        ...
        setmatchingBonusMbalance(balance);
      }
    }
  }

  Future<void> checkmatchingBonus() async {
    try {
      String token = sharedPreferences.getString('token').toString();
      final url = Uri.parse(Api.matchingBonous);
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (status) {
          ...
          if (data != null) {
            for (var a in data) {
              MatchingBonusM matchingBonusM = MatchingBonusM.fromMap(a);

              _list.add(matchingBonusM);
            }
          }
        }
        notifyListeners();
...
class MatchingBonusM {
  ...
  static MatchingBonusM fromMap(Map<String, dynamic> map) {
